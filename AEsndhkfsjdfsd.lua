...
local Library = require(game:GetService("ReplicatedStorage").Framework.Library)
for i, v in pairs(Library.Save.Get().Pets) do
if v.e and PettoRarity[v.id] ~= 'Mythical' and not getgenv().EMERGENCYSTOP then
    ...
    repeat wait()
        if GetPetTable(v.uid).powers then
            for a, b in pairs(GetPetTable(v.uid).powers) do
                warn(b[1], b[2])
                print(Wanted[b[1]])
                if Wanted[b[1]] ~= nil and not haspower and not getgenv().EMERGENCYSTOP then
            ...
            if not haspower and not getgenv().EMERGENCYSTOP then
    ...
    until haspower == true or Library.Save.Get().Diamonds < 10000 or getgenv().EMERGENCYSTOP
end
end
